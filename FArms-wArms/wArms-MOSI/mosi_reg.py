...
def load_audio_file(audio_path):
    """Load audio file with fallback methods for different torchaudio versions"""
    try:
        # Try standard torchaudio.load
        waveform, sr = torchaudio.load(audio_path, backend="soundfile")
        return waveform, sr
    except:
        try:
            # Try without backend argument
            waveform, sr = torchaudio.load(audio_path)
            return waveform, sr
        except:
            # Last resort: use soundfile directly
            import soundfile as sf
            data, sr = sf.read(audio_path)
            waveform = torch.FloatTensor(data).unsqueeze(0) if len(data.shape) == 1 else torch.FloatTensor(data).T
            return waveform, sr
...
class MOSIDatasetRegression(Dataset):
    ...
    def __init__(self, audio_dir, video_dir, text_dir, split_file, split='train',
                 ...
                 max_text_len=512, seed=42, max_frames_per_video=30):
    ...
    def load_video(self, sample_name):
        """Load video frames with timeout and error handling"""
        ...
        try:
            ...
            frames = []
            frame_count = 0
            max_attempts = self.max_frames_per_video * 2  # Try reading more, but limit
            attempts = 0
            
            while len(frames) < self.max_frames_per_video and attempts < max_attempts:
                ret, frame = cap.read()
                attempts += 1
